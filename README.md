# 💤 LazyVim

A starter template for [LazyVim](https://github.com/LazyVim/LazyVim).
Refer to the [documentation](https://lazyvim.github.io/installation) to get started.

Extra setup for Linux:

- sudo apt install fzf
- Install Embellish from app store and pick a font
  - (Windows) download font, click to install, select it in terminal settings
- git clone <https://github.com/neovim/nvim-lspconfig> ~/.config/nvim/pack/nvim/start/nvim-lspconfig
- Install brew: - test -d ~/.linuxbrew && eval "$(~/.linuxbrew/bin/brew shellenv)"
test -d /home/linuxbrew/.linuxbrew && eval "$(/home/linuxbrew/.linuxbrew/bin/brew shellenv)"
  echo "eval \"\$($(brew --prefix)/bin/brew shellenv)\"" >> ~/.bashrc - brew install ripgrep
- Lazygit:
  LAZYGIT*VERSION=$(curl -s "https://api.github.com/repos/jesseduffield/lazygit/releases/latest" | \grep -Po '"tag_name": *"v\K[^"]*')
curl -Lo lazygit.tar.gz "https://github.com/jesseduffield/lazygit/releases/download/v${LAZYGIT_VERSION}/lazygit*${LAZYGIT_VERSION}\_Linux_x86_64.tar.gz"
  tar xf lazygit.tar.gz lazygit
  sudo install lazygit -D -t /usr/local/bin/
- apt-get install fd-find

Git diff (side-by-side) in Neovim:

- `<leader>gd` toggle Diffview (working tree vs index)
- `<leader>gD` toggle Diffview (staged)
- `<leader>gq` close Diffview
  To close, use :DiffviewClose
